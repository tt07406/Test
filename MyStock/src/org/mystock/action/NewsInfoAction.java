...
public class NewsInfoAction extends ActionSupport {
	...
	public String update() {
		...
		if (admin == null){
			...
			news = new NewsInfo(newsInfoId,name,content,
					service.searchNewsInfo(newsInfoId).getNewsInfoTime(),author,type.toString(),admin.getAdminName());//创建时间不变
			try {//更新数据库
	...
	public String insert(){
		...
		if (admin == null){
			...
			news = new NewsInfo(newsInfoId,name,content,
					new Date(new java.util.Date().getTime()),author,type.toString(),admin.getAdminName());//创建时间为当前时间
			try {//更新数据库
